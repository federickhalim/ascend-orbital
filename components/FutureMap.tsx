...
import { FUTURE_START, SECONDS_PER_LEVEL, LEVELS_PER_ERA } from "@/config/eraThresholdConfig";
...
const ASSET_SIZE = 265;
const HALF_SIZE = ASSET_SIZE / 2;

export default function FutureMap({ totalFocusTime }: FutureMapProps) {
  const [prevEraLevel, setPrevEraLevel] = useState(1);
  const scaleAnim = useRef(new Animated.Value(1)).current;
  const opacityAnim = useRef(new Animated.Value(1)).current;

  const relativeFocusTime = totalFocusTime - FUTURE_START;
